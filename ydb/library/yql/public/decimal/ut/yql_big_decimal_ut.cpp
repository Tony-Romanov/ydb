...
#include <ydb/library/yql/public/decimal/yql_big_decimal_serialize.h>
...
Y_UNIT_TEST_SUITE(TYqlBigDecimalTest) {
    void SimplePositiveTest(TInt256 v, ui8 precision, ui8 scale, const TString& expected) {
        TString result = ToString(v, precision, scale);
        UNIT_ASSERT_VALUES_EQUAL(result, expected);
        TInt256 parsed = FromString(result, precision, scale);
        UNIT_ASSERT(parsed == v);
    }

    void SimpleNegativeFormatTest(TInt256 v, ui8 precision, ui8 scale) {
        TString result = ToString(v, precision, scale);
        UNIT_ASSERT_VALUES_EQUAL(result, "");
    }

    void SimpleSerializeAndDeserialize(TInt256 v, size_t expectedSize) {
        char buff[sizeof(TInt256)];
    ...
    Y_UNIT_TEST(TestHugeNumberFormat) {
        TInt256 max256 = Inf() - 1;
        const char max256String[] = "9999999999999999999999999999999999999999999999999999999999999999999999999999"; // 76 digits
        static_assert(sizeof(max256String) == 77, "sizeof(max256String) == 77");
        SimplePositiveTest(max256, MaxPrecision, 0, max256String);
        SimplePositiveTest(max256 + 1, MaxPrecision, 0, "inf");

        TInt256 min256 = -Inf() + 1;
        const char min256String[] = "-9999999999999999999999999999999999999999999999999999999999999999999999999999";
        static_assert(sizeof(min256String) == 78, "sizeof(min120String) == 78");
        SimplePositiveTest(min256, MaxPrecision, 0, min256String);
        SimplePositiveTest(min256 - 1, MaxPrecision, 0, "-inf");

        // take spot for sign and zero before dot
        const char min256StringAfterDot[] = "-0.9999999999999999999999999999999999999999999999999999999999999999999999999999"; // 35 by nine + leading zero
        static_assert(sizeof(min256StringAfterDot) == 80, "sizeof(min256StringAfterDot) == 80");
        SimplePositiveTest(min256, MaxPrecision, MaxPrecision, min256StringAfterDot);
    ...
    Y_UNIT_TEST(TestInfinityValues) {
        ...
        UNIT_ASSERT(FromString("9999999999999999999999999999999999999999999999999999999999999999999999999999.5", 76, 0) == Inf());
        UNIT_ASSERT(FromString("-9999999999999999999999999999999999999999999999999999999999999999999999999999.5", 76, 0) == -Inf());
    }

    Y_UNIT_TEST(TestInvalidValues) {
        UNIT_ASSERT(IsValid("+99999999999999999999999999999999999999991234567890.039493804903849038490312345678909999999999999999990"));
    ...
    Y_UNIT_TEST(TestToStringOfNonNormal) {
        // above Inf
        for (TInt256 i = Inf() + 2, end = Inf() + 100; i < end; i++) {
    ...
    Y_UNIT_TEST(TestSerializeAndDeserialize) {
        ...
        SimpleSerializeAndDeserialize(-Inf() + 1, 33U);
        SimpleSerializeAndDeserialize(-Inf() + 2, 33U);
